...
应用资源关闭会共享同一次正在执行的清理任务。调用方取消等待时，清理仍继续，完成后再传播取消；某个资源自行抛出 `CancelledError` 时仍尝试关闭其余资源。成功关闭的资源不重复关闭，失败或自行取消的资源可在下次关闭时重试。此机制不为第三方资源的 `close()` 自动设置超时，资源自身仍需提供合理的关闭时限。
