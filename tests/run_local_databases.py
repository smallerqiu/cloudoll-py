...
def main():
    ...
    parser.add_argument(
        "--native-only", action="store_true", help="Skip AWS wrapper cases"
    )
    parser.add_argument(
        "--test-file",
        choices=[
        ...
        default="tests/integration/test_databases.py",
    )
    ...
    interpreters = [str(Path(p).absolute()) for p in options.interpreters or []] or [
        sys.executable
    ]
    ...
    with tempfile.TemporaryDirectory(prefix="cloudoll-db-") as directory:
        ...
        try:
            subprocess.run(
                [
                    str(mysql_bin / "mysqld"),
                    "--no-defaults",
                    "--initialize-insecure",
                    "--datadir=" + str(mysql_data),
                ],
                check=True,
                timeout=90,
                capture_output=True,
            ...
            with (tmp / "servers.log").open("w+") as log:
                ...
                for python in interpreters:
                    ...
                    test_command = [python, "-m", "pytest", "-q", options.test_file]
                    if options.native_only:
                        test_command.extend(["-k", "not aws"])
                    subprocess.run(
                        test_command, cwd=root, env=env, check=True, timeout=180
                    )
