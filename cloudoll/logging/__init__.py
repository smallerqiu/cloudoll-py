...
class HandleLog:
    ...
    def getLogger(self, __file):
        ...
        # cur_path = os.path.dirname(os.path.realpath(__file))  # 当前项目路径
        # log_path = os.path.join(os.path.dirname(cur_path),'logs')  # log_path为存放日志的路径
        ...
        self.__all_log_path = os.path.join(log_path, self.__now_time + "-all" +
                                           ".log")  # 收集所有日志信息文件
        self.__error_log_path = os.path.join(log_path, self.__now_time +
                                             "-error" + ".log")  # 收集错误日志信息文件
        self.__logger = logging.getLogger(__file__)  # 创建日志记录器
    ...
    def __set_color_formatter(console_handle, color_config):
        ...
        :return:
        """
        formatter = colorlog.ColoredFormatter(FORMATS["console_format"],
    ...
    def __set_log_formatter(file_handler):
        ...
        """
        formatter = logging.Formatter(FORMATS["log_format"],
    ...
    def __console(self, level, message):
        """构造日志收集器"""
        all_logger_handler = self.__init_logger_handler(
            self.__all_log_path)  # 创建日志文件
        error_logger_handler = self.__init_logger_handler(
        ...
        self.__set_color_formatter(console_handle, COLORS)
