...
class RowStream(AsyncIterator[dict[str, Any]]):
    def __init__(
        self, cursor: Any, connection: Any, batch_size: int, timeout: Optional[float]
    ) -> None:
        self._cursor = cursor
    ...
    async def __anext__(self) -> dict[str, Any]:
        if not self.active:
            raise RuntimeError(
                "Row stream is closed; iterate inside its async with block"
            )
        ...
        if not self._buffer and not self.exhausted:
            try:
                rows = await asyncio.wait_for(
                    self._cursor.fetchmany(self._batch_size), self._timeout
                )
