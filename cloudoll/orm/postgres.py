...
class _ServerCursor:
    ...
    def __init__(self, cursor: Any, name: str) -> None:
        self.cursor = cursor
        self.name = name
    ...
    async def fetchmany(self, size: int) -> list[dict[str, Any]]:
        ...
        return cast(
            list[dict[str, Any]], await self.cursor.fetchall()
        )  # Only this server-side batch is buffered.
...
class Postgres(AsyncEngine):
    ...
    async def _execute(
        ...
        async with connection.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            return await cursor_result(
                cursor, query_type, size, postgres=True, batch_count=count
            )

    async def create_engine(self, **kw: Any) -> Postgres:
        self.configure(kw)
        tls = {
            name: kw[name]
        ...
        self.pool = await aiopg.create_pool(
            ...
            dbname=kw.get("db"),
            timeout=self.connect_timeout,  # type: ignore[arg-type]  # aiopg passes None to asyncio.wait_for (no timeout).
            echo=False,
